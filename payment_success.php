...
<main>
    <?php if ($transaction): ?>
        ...
        <div style="background: #fff; padding: 2rem; border-radius: 15px; box-shadow: 0 5px 15px rgba(139, 90, 60, 0.1); margin: 2rem 0;">
            ...
            <!-- Tombol Invoice (hanya muncul jika status sudah confirmed) -->
            <?php if (strtolower($transaction['status']) === 'confirmed'): ?>
                <div style="text-align: center; margin-top: 2rem;">
                    <a href="invoice.php?transaction_id=<?php echo $transaction['id']; ?>" 
                       class="btn" 
                       style="background-color: #d4a373; color: white; padding: 0.8rem 1.5rem; border-radius: 8px; text-decoration: none;">
                       Lihat Invoice
                    </a>
                </div>
            <?php endif; ?>
