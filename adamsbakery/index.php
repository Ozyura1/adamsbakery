...
$featured_products = [];
$categories = ['Roti Manis', 'Roti Tawar', 'Kue Kering', 'Kue Ulang Tahun'];
foreach ($categories as $category) {
    $result = $conn->query("SELECT * FROM products WHERE kategori = '$category' ORDER BY id LIMIT 3");
    ...
    <div style="flex: 1; min-width: 0;">
        <main>
            ...
            <section style="text-align: center; padding: 3rem 0; background: linear-gradient(135deg, #f4e4c1 0%, #e8d5b7 100%); border-radius: 15px; margin-bottom: 3rem;">
                ...
                <img src="assets/adamrot.jpg" alt="Roti Segar Adam Bakery" 
                     style="max-width: 400px; width: 100%; border-radius: 15px; box-shadow: 0 8px 25px rgba(139, 90, 60, 0.2);">
            </section>
                ...
                <div class="product-grid">
                    <?php while ($package = $featured_packages->fetch_assoc()): ?>
                        <div class="package-card">
                            <img src="/placeholder.svg?height=180&width=280" 
                                 alt="<?php echo $package['nama']; ?>" 
                        ...
                        <div style="background: #fff; padding: 1.5rem; border-radius: 10px; text-align: center;">
                            ...
                            <?php if ($review['review_text']): ?>
                                <p style="font-style: italic; color: #6b5b47; margin-bottom: 1rem;">
                                    "<?php echo substr($review['review_text'], 0, 100) . '...'; ?>"
                                </p>
                            <?php endif; ?>
                            
                            <h4 style="color: #8b5a3c; margin-bottom: 0.5rem;"><?php echo $review['nama_reviewer']; ?></h4>
                            <small style="color: #8b5a3c;"><?php echo $review['item_name']; ?></small>
                        </div>
                    <?php endwhile; ?>
                </div>
            ...
            <?php if (isset($_SESSION['cart']) && !empty($_SESSION['cart'])): ?>
                <div style="position: fixed; bottom: 20px; right: 20px; background: #d4af8c; color: white; padding: 1rem; border-radius: 50px; box-shadow: 0 4px 12px rgba(139, 90, 60, 0.3); z-index: 1000;">
